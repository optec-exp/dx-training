...
export function ActionBar({ year, month, disabled }: Props) {
  ...
  const downloadExcel = () => {
    window.open(`/api/export/excel?year=${year}&month=${month}&lang=${lang}`, "_blank");
  };

  const downloadPdf = () => {
    window.open(`/api/export/pdf?year=${year}&month=${month}&lang=${lang}`, "_blank");
  };

  const notifySlack = useCallback(async () => {
    if (!confirm(t("slackConfirm", { year, month }))) return;
    ...
    try {
      ...
      if (res.ok) {
        setToast({
          type: "success",
          message: t("slackSuccess"),
          detail: t("slackSuccessDetail", {
            year,
            month,
    ...
    <div className="fixed top-6 left-1/2 -translate-x-1/2 z-50 max-w-xl w-[90%]">
      <div
        className={`flex items-start gap-3 rounded-xl border-2 px-5 py-4 shadow-xl ${
          isErr ? "border-rose-300 bg-rose-50" : "border-emerald-300 bg-emerald-50"
        }`}
      >
        <div className="text-2xl">{isErr ? "❌" : "✅"}</div>
        <div className="flex-1">
          <div
            className={`font-semibold ${isErr ? "text-rose-900" : "text-emerald-900"}`}
          >
            {toast.message}
          </div>
          {toast.detail && (
            <div
              className={`mt-1 text-sm ${isErr ? "text-rose-700" : "text-emerald-700"}`}
            >
              {toast.detail}
            </div>
          )}
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className={`rounded-lg px-2 py-1 text-xs font-medium ${
              isErr ? "text-rose-700 hover:bg-rose-100" : "text-emerald-700 hover:bg-emerald-100"
            }`}
