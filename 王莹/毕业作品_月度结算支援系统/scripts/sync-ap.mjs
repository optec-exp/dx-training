// 应付账龄同步：支付(1013/1015) 未払 → settlement.ar_ap_aging(类型=应付)。
// 运行：node scripts/sync-ap.mjs 2026-06-05
...
async function fetchOut(id, token) {
  const out = []; let lastId = 0;
  for (;;) {
    const q = `円換算差額未払金 > 1 and $id > ${lastId} order by $id asc limit 500`;
...
for (const [id, token] of [[env.KINTONE_APP_EXP_PAYMENTS_ID, env.KINTONE_APP_EXP_PAYMENTS_TOKEN], [env.KINTONE_APP_EC_PAYMENTS_ID, env.KINTONE_APP_EC_PAYMENTS_TOKEN]]) {
  const recs = await fetchOut(id, token);
  for (const rec of recs) {
    const amt = num(rec["円換算差額未払金"]);
    if (amt <= 1) continue;
    const due = v(rec["支払期日"]);
    let daysOver = due ? Math.floor((ref - new Date(due + "T00:00:00+09:00")) / 86400000) : 0;
    const overdue = daysOver > 0;
    const bucket = daysOver <= 30 ? "0-30" : daysOver <= 60 ? "31-60" : daysOver <= 90 ? "61-90" : "90+";
    rows.push({ 期间: refStr, 类型: "应付", 客户供应商: v(rec["支払先"]), 金额: amt, 账龄桶: bucket, 是否超期: overdue });
  }
}
const H = { apikey: SKEY, Authorization: `Bearer ${SKEY}`, "Content-Profile": "settlement" };
await fetch(`${SUPA}/rest/v1/ar_ap_aging?类型=eq.应付`, { method: "DELETE", headers: H });
for (let i = 0; i < rows.length; i += 200) {
  ...
  if (!w.ok) throw new Error(`写 ${w.status}: ${await w.text()}`);
}
const fmt = (n) => Math.round(n).toLocaleString();
const total = rows.reduce((s, r) => s + r.金额, 0), over = rows.filter((r) => r.是否超期);
console.log(`✅ 应付账龄：${rows.length} 笔 ¥${fmt(total)}，超期 ${over.length} 笔 ¥${fmt(over.reduce((s, r) => s + r.金额, 0))}`);
