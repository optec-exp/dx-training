...
export interface AcctMove { 银行: string; 口座番号: string; 转入: number; 转出: number }
export interface CashConstituent { 业务出金: number; 贩管费出金: number; 内部移动?: number; 转入?: number; 转出?: number; 账户?: AcctMove[] }
