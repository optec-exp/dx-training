...
export async function reconcileBill(bill: ParsedBill): Promise<ReconResult> {
  ...
  // 提单号(运单号/MAWB)→OPT 映射（kc_cases.提单号），用于按运单号纠正/兜底 OPT
  ...
  // 逐行解析真实 OPT：① 运单号(提单号) 命中 MAWB→OPT 优先(可纠正账单写错的品名OPT)；② 账单单号本身命中 MAWB；③ 否则账单单号当 OPT。再按真实 OPT 聚合。
  const resolved = new Map<string, { amount: number; viaBL: boolean; 原单号: string }>();
  for (const ln of bill.lines) {
    const billKey = ln.opt_no || "";
    const blHit = ln.提单号 ? blToOpt.get(norm(ln.提单号)) : undefined;
    const keyHit = billKey ? blToOpt.get(norm(billKey)) : undefined;
    const realOpt = blHit || keyHit || billKey;
    if (!realOpt) continue; // 既无OPT又无可解析运单号 → 跳过
    const viaBL = realOpt !== billKey; // 经运单号纠正/兜底得到的 OPT
    const cur = resolved.get(realOpt);
    if (cur) { cur.amount += ln.金额; cur.viaBL = cur.viaBL || viaBL; if (!cur.原单号 && billKey) cur.原单号 = billKey; }
    else resolved.set(realOpt, { amount: ln.金额, viaBL, 原单号: billKey || realOpt });
  }
  ...
  for (const [opt, info] of resolved) {
    ...
    const blNote = info.viaBL ? `经运单号定位OPT(账单单号${info.原单号})` : "";
