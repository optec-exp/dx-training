...
### ⑤贩管费排除口径改为费用类型白名单（役員即便集計対象外也保留）
- **王莹**：费用是否抓取主要取决于【费用类型】，即便勾选了集計対象外，那条役員関連費用也要被记录。
- **数据核实**：集計対象外✓ 的记录=对象外12条/役員1条(¥319,720)/(空费用类型)14条(¥548万)；収入項目✓ 全是对象外/(空)。
- **改了什么**：syncSga 的 isExcluded 从「税金/对象外/集計対象外/収入項目」改为 **`!KEEP_FEES.has(fee)`**——只保留费用类型∈5类(人件費/事業活動費/事業維持費/人材·IT投資/役員関連費用)，其余(税金/对象外/空未知)排除；不再看集計対象外/収入項目勾选框。删除未再使用的 checked helper。lib/sync.ts + scripts/sync-sga.mjs 同步,已重同步03/04/05。
- **效果**：役員 2026-05 ¥319,720（那条集計対象外的役員记录现已捞入）、2026-04 ¥5,775,639(不变)；(空)费用类型那14条¥548万仍排除(不在5类)；对象外/税金仍排除。
- **文件**：lib/sync.ts、scripts/sync-sga.mjs
- **验证**：两月类型均只剩5类;役員5月319,720/4月577万;改动文件tsc无新错误;/profit 200。✅

### ⑤贩管费役員関連費用捞取修正（公司级费用无部署按分）
- **王莹**：全社贩管费役員関連費用为何是 0？
- **根因**：syncSga 只读 `部署按分` 子表的 `部署按分費用JPY`（且跳过 0 值）。役員関連費用是公司级费用（法定福利費/税金/社保/给与），**无部署按分**，金额在记录级 `円換算費用` 字段→syncSga 整个漏掉→sg_a_lines 无役員行→全社役員=0。与取引日口径改动无关，一直存在。
- **改了什么**：syncSga 在 部署按分 合计=0 且 费用类型=役員関連費用 时，用记录级 `円換算費用` 兜底存一行（region=null，由 getSgaForMonth 的 5/5 中日逻辑处理；是否除外照常判定）。收窄到仅役員，避免引入无识别费用类型的「(未知)」记录。lib/sync.ts + scripts/sync-sga.mjs 同步。已重同步 03/04/05。
- **数据**：役員 2026-04 ¥5,775,639（取引日 4-30→权责归 4 月）、2026-05 ¥0（唯一 5 月取引日的役員是集計対象外，正确排除）。sg_a_lines 与 Kintone 取引日∈月 完全对账，无重复。
- **排除逻辑确认**：集計対象外勾选 且 费用类型∈4类 = 0 条→按集計対象外 checkbox 排除安全，不误伤 4 类数据。
- **文件**：lib/sync.ts、scripts/sync-sga.mjs
- **验证**：2026-04 全社贩管费含役員¥577万;sg_a_lines对账Kintone一致;改动文件 tsc 无新错误;/profit 200。✅
